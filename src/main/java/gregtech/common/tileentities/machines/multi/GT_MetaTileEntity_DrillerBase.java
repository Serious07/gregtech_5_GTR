...
public abstract class GT_MetaTileEntity_DrillerBase extends GT_MetaTileEntity_MultiBlockBase {
    ...
    /*
     * Modified by Serious07 to chunckloader start Work with global fake player
     */
    protected FakePlayer FindFakePlayerInWorlds() {
        World world;
        FakePlayer tmpFakePlayer;
        
        for(int i=0; i<MinecraftServer.getServer().worldServers.length; i++){
            world = MinecraftServer.getServer().worldServers[i];
            tmpFakePlayer = FakePlayerFactory.getMinecraft((WorldServer)world);
            
            if(tmpFakePlayer != null) {
                return tmpFakePlayer;
            }
        }
    ...
    protected FakePlayer getFakePlayer(IGregTechTileEntity aBaseTile) {
        if(mFakePlayer == null) mFakePlayer = FindFakePlayerInWorlds();
        mFakePlayer.setWorld(aBaseTile.getWorld());
    	mFakePlayer.setPosition(aBaseTile.getXCoord(), aBaseTile.getYCoord(), aBaseTile.getZCoord());
        return mFakePlayer;
    }
