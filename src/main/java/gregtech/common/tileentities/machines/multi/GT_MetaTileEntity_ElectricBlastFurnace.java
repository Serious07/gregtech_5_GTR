...
public class GT_MetaTileEntity_ElectricBlastFurnace extends GT_MetaTileEntity_MultiBlockBase {
	...
	public boolean checkRecipe(ItemStack aStack) {
		...
		if (tInputList.size() > 0) {
			...
			GT_Recipe tRecipe = GT_Recipe.GT_Recipe_Map.sBlastRecipes.findRecipe(getBaseMetaTileEntity(), false,
			...
			for (int zPos = tZ - 1; zPos <= tZ + 1; zPos++) {
				...
				for (int yPos = tY + 1; yPos <= tY + 2; yPos++) {
					...
					if (tUsedMeta >= 12 && tUsedMeta <= 14
						...
						aBaseMetaTileEntity.getWorld().setBlock(xPos, yPos, zPos, GregTech_API.sBlockCasings5,
								tUsedMeta - 12, 3);
					}
				}
			}
	...
	public String[] getInfoData() {
		...
		return new String[] { StatCollector.translateToLocal("GT5U.multiblock.Progress") + ": "
				...
				StatCollector.translateToLocal("GT5U.multiblock.problems") + ": " + EnumChatFormatting.RED
						...
						+ " %",
				StatCollector.translateToLocal("GT5U.EBF.heat") + ": " + EnumChatFormatting.GREEN + mHeatingCapacity
						+ EnumChatFormatting.RESET + " K",
